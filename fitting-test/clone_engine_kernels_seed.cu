...
__device__ void updateCandsWithInfo_seed(Track *candidates, int16_t *ntracks_per_seed,
  ...
  for (int ebin = 0; ebin < Config::nEtaBin; ebin++) {
    ...
    for (int iseed_ev = iseed_plex; 
      ...
      for (int ilay = nlayers_per_seed; ilay <= Config::nLayers; ++ilay) {
        ...
        // FIXME: there is two breaks because of the loop over cands
        // inside the loop over layer.
        // That's ugly
        if (ilay >= Config::nLayers) break;
    ...
    int *maxSize, int N)
{
  ...
  findInLayers_kernel_seed <<<grid, block, 0, stream>>>
      (layers, 
       ...
       SeedIdx, CandIdx, geom, maxSize, N, Config::nlayers_per_seed);
  /*cudaCheckErrorSync();*/
}
