    ...
    GPlexHitIdx &XHitArr, const int itrack, const int N)
{
  if (itrack < N) {
    ...
    trove::coalesced_ptr<int2> phi_bin_infos_ptr {L.m_phi_bin_infos.data()};
    ...
    for (int zi = zb1; zi < zb2; ++zi)
    {
      for (int pi = pb1; pi < pb2; ++pi)
      {
        ...
        /*auto bin_bounds  = L.m_phi_bin_infos[zi*Config::m_nphi + pb];*/
        int2 bin_bounds = *(phi_bin_infos_ptr[zi*Config::m_nphi + pb]).m_ptr;

        /*for (int hi = bin_bounds.first; hi < bin_bounds.second; ++hi)*/
        for (int hi = bin_bounds.x; hi < bin_bounds.y; ++hi)
        {
          ...
          if (hit_size_itrack < GPlexHitIdxMax)
          {
            XHitArr(itrack, hit_size_itrack++, 0) = hi;
          }
        }
      }
    }
    XHitSize[itrack] = hit_size_itrack;
  }
}
